...
(function () {
    ...
    // Hosts that are automatically enabled
    function _getEnabledHosts() {
        return localStorage.whitelist.split("\n").map(function (host) { return host.trim(); });
    }
    ...
    function _toggleDomain(tab) {
        ...
        if (_domainIsActive(host)) {
            localStorage.hosts[host] = false;
            _deactivate(tab.id);
            return;
        }
        localStorage.hosts[host] = true;
        _activate(tab.id);
    }
    ...
    function _domainIsActive(host) {
        ...
        if (typeof localStorage.hosts[host] === 'undefined') {
            return _getEnabledHosts().some(function (h) { return h == host; });
        } else {
            return !!localStorage.hosts[host];
        }
    }
